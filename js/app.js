...
document.addEventListener('DOMContentLoaded',async()=>{setTheme();initShell();void initWeather();await Promise.all([hydrateNews(),hydrateVideos(),refreshBreaking(),loadAdvertisements()]);connectLiveUpdates();const type=document.body.dataset.template;if(type==='home')home();if(type==='category')await categoryPage();if(type==='article')await articlePage();if(type==='search')searchPage();if(type==='live')livePage();if(type==='media')mediaPage(document.body.dataset.media);if(type==='static')staticPage();});
