...
export class NewsAggregator {
  ...
  private async persist(article:NormalizedNewsArticle,sourceId:string):Promise<'inserted'|'updated'|'duplicate'>{
    ...
    await this.saveTags(saved.id,ai.tags); await this.saveLiveUpdate(saved.id,article); await this.saveVideo(saved.id,slug,article);
  ...
  private async saveVideo(articleId:string,articleSlug:string,article:NormalizedNewsArticle){
    const videoUrl=article.videoUrl||article.youtubeUrl;if(!videoUrl)return;
    const {error}=await this.db.from('videos').upsert({title:article.title,slug:`${articleSlug}-video`,description:article.description||null,video_url:videoUrl,youtube_url:article.youtubeUrl||null,thumbnail_url:article.image||null,duration_seconds:article.durationSeconds||null,article_id:articleId,status:'published',published_at:article.publishedAt},{onConflict:'slug'});
